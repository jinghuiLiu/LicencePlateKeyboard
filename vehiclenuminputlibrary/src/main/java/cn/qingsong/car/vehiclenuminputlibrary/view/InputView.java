...
import cn.qingsong.car.vehiclenuminputlibrary.R;
...
public class InputView extends ConstraintLayout {
    ...
    private boolean mIsNewEnergyType = false;
    ...
    private void onInited(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //
        if (labelVisible == 0) {
            TextView field = mFieldViewGroup.getLastField();
            field.setText("");
    ...
    public  void changeNewEnergyType(){
        TextView field = mFieldViewGroup.getLastField();
        if ("新\n能\n源".equals(field.getText().toString())){
            field.setText("");
        }
    ...
    public void removeLastCharOfNumber() {
         TextView last = null;
        if (mIsNewEnergyType){
            last = mFieldViewGroup.getLastFilledFieldOrNull();
        }else {
            last = mFieldViewGroup.getNormalVehicleFieldOrNull();
        }
    ...
    public void setmIsNewEnergyType(boolean mIsNewEnergyType) {
        this.mIsNewEnergyType = mIsNewEnergyType;
    }
    ...
    public  TextView getLastFields(){
        return mFieldViewGroup.getLastField();
    }
    ...
    private void invalidateSelectedDrawable(Canvas canvas) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (selected == mFieldViewGroup.getFieldAt(0)) {
                provinceTextIsEmpty((TextView) selected);
            }

            //energy
            if (selected == mFieldViewGroup.getFieldAt(7)) {
                TextView selected1 = (TextView) selected;
    ...
    private void provinceTextIsEmpty(TextView textView){
        if (TextUtils.isEmpty(textView.getText().toString())){
            if (mProvinceUnSelectedDrawable != null) {
                ...
                textView.setBackground(mProvinceUnSelectedDrawable);
            }
        }else {
            if (mProvinceSelectedDrawable != null) {
                textView.setTextColor(Color.WHITE);
                textView.setBackground(mProvinceSelectedDrawable);
            }
        }
    }
